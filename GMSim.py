...
def RJB2REPI(M, R):
    ...
    if M >= 3.25 and M <= 6.75:
        ...
        # Compute average
        mean = (
                           25.2859086858311 - 24.7579132762745 * M + 9.1067957463996 * M ** 2 - 1.4924537794753 * M ** 3 + 0.0943269381529 * M ** 4) + \
               d * (
                           1.0258015468051 - 0.9546610173338 * M + 0.3306850146342 * M ** 2 - 0.0507022784386 * M ** 3 + 0.0029166620192 * M ** 4) + \
               d ** 2 * (
                           -0.0087847623757 + 0.008078532233 * M - 0.0027616254404 * M ** 2 + 0.0004175139082 * M ** 3 - 0.0000236750349 * M ** 4) + \
               d ** 3 * (
                           0.0000210744968 - 0.0000192014444 * M + 0.0000064976695 * M ** 2 - 0.0000009719658 * M ** 3 + 0.0000000545396 * M ** 4)

        # Compute sigma
        sigma = (
                            50.4650608321051 - 48.0676507162484 * M + 17.1052739585892 * M ** 2 - 2.7022941754681 * M ** 3 + 0.1613658945296 * M ** 4) + \
                d * (
                            0.7371820139324 - 0.6752613553 * M + 0.2293488841656 * M ** 2 - 0.0343077682523 * M ** 3 + 0.00191302107 * M ** 4) + \
                d ** 2 * (
                            -0.0056666537501 + 0.0051882394919 * M - 0.0017625955284 * M ** 2 + 0.0002639958786 * M ** 3 - 0.0000147580962 * M ** 4) + \
                d ** 3 * (
                            0.0000113403782 - 0.000010388538 * M + 0.0000035364421 * M ** 2 - 0.0000005318502 * M ** 3 + 0.0000000299318 * M ** 4)
        ...
        # Compute average
        mean = (
                           41330.5714998843 - 22733.3089137796 * M + 4692.74801622372 * M ** 2 - 431.089171902869 * M ** 3 + 14.8828551078011 * M ** 4) + \
               d * (
                           -2416.16571885248 + 1313.68542796199 * M - 267.398312525097 * M ** 2 + 24.1469682260178 * M ** 3 - 0.8160573655126 * M ** 4) + \
               d ** 2 * (
                           20.7418579312278 - 11.2634077165846 * M + 2.2898378394305 * M ** 2 - 0.2065409463611 * M ** 3 + 0.0069730227999 * M ** 4) + \
               d ** 3 * (
                           -0.0518782359661 + 0.0281154133727 * M - 0.0057044823582 * M ** 2 + 0.00051353589 * M ** 3 - 0.0000173053232 * M ** 4)

        # Compute sigma
        sigma = (
                            12207.5188429977 - 7001.4407532747 * M + 1515.67332601025 * M ** 2 - 146.912852179082 * M ** 3 + 5.388523704157 * M ** 4) + \
                d * (
                            -1318.61674483966 + 718.285967089764 * M - 146.541924947989 * M ** 2 + 13.2688348378962 * M ** 3 - 0.4497899372784 * M ** 4) + \
                d ** 2 * (
                            14.9495278339686 - 8.1301182336872 * M + 1.6558268525748 * M ** 2 - 0.1496655480247 * M ** 3 + 0.0050646642932 * M ** 4) + \
                d ** 3 * (
                            -0.041450318097 + 0.0224827452608 * M - 0.004566514641 * M ** 2 + 0.0004116109653 * M ** 3 - 0.0000138901781 * M ** 4)
...
def REPI2RJB(M, Repi):
    # Converts between Rjb and Repicenter
    # Inputs:
    #   M - Moment Magnitude
...
def GroundMotionSim(M, Dist, Rval, Vs30, F, n):
    ...
    #             LOGN[i][j-1]= 1/(j* np.sqrt(2*np.pi) * delta[i] ) \
    #             * np.exp(-1 * ((np.log(j)+np.log(om_0) - ln_beta_t[i])**2)/(2 * delta[i]**2))
    ...
    # Possible error, should be <= and > instead of < and >=
    for i in range(len(t)):
    ...
    # Calculate PGA for each simulation and get the mean PGA over all simulations
    maxeach = abs(ACC).max(axis=0)
    maxmean = np.mean(maxeach)

    # Select the ground motion with PGA nearest the mean PGA
    meandiff = abs(maxeach - maxmean)
    selectMotion = np.where(meandiff == np.min(meandiff))[0][0]

    ACC = ACC[:,selectMotion]

    # Convert Acc units to be in (g)
    ACC /= 981

    # Save acceleration history to csv
    # with open('Acchist', 'w') as csvfile:
    #     writer=csv.writer(csvfile, delimiter=',')
    #     writer.writerows(zip(t, ACC.T.flatten()))
